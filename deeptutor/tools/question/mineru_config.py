...
This is the single read-side adapter between the persisted ``mineru.json``
settings (owned by :class:`RuntimeSettingsService`) and the parser backends
in :mod:`deeptutor.tools.question.mineru_backend`. Keeping it here means the
parser code never touches the settings storage shape directly — it asks for a
...
def resolve_mineru_config() -> MinerUConfig:
    """Load the effective MinerU config from ``mineru.json`` (+ env overrides)."""
